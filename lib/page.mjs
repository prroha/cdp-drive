...
export async function collectLogs(socket, send, seconds, sessionId) {
  const events = [];
  socket.addEventListener("message", (event) => {
    const message = JSON.parse(event.data);
    // Only this page's session: a second attach would otherwise fold another
    // target's console into these results.
    if (sessionId != null && message.sessionId !== sessionId) {
      return;
    }
    const described = describeLogEvent(message);
    if (described) {
