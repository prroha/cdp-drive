...
export const LAUNCH_GUARD_MS = 45000;
...
const FLAG_MINIMUMS = { "--port": 1, "--tab": 0, "--timeout": 1 };

function numberOr(flag, raw) {
  const value = Number(raw);
  const minimum = FLAG_MINIMUMS[flag] ?? 0;
  if (!Number.isFinite(value) || value < minimum) {
    return { error: `${flag} expects a number of at least ${minimum}, got ${raw}` };
  }
  return { value };
}
...
export function parseArgs(argv, env = {}) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    // `--flag=value` is the way to pass a value that itself begins with `--`,
    // since the space form cannot tell one from a mistyped flag.
    const [flagName, ...rest] = arg.split("=");
    if (rest.length > 0 && FLAGS_WITH_VALUES.has(flagName)) {
      const error = applyValueFlag(flagName, rest.join("="), opts, frames);
      if (error) {
        return { ...baseResult(opts, frames, positional), error };
      }
      continue;
    }
    if (FLAGS_WITH_VALUES.has(arg)) {
      ...
      if (value == null || value.startsWith("--")) {
        return { ...baseResult(opts, frames, positional), error: `${arg} needs a value` };
      }
...
export function hangMsFor({ cmd, opts, logSeconds, baseHangMs = DEFAULT_HANG_MS }) {
  if (cmd === "launch") {
    return Math.max(baseHangMs, LAUNCH_GUARD_MS);
  }
...
// `localhost` resolves to ::1 first on many systems while the browser listens on
// IPv4 only, so every loopback spelling tries both.
export function endpoints(opts, env = {}) {
  if (env.CDP_URL) {
    return [env.CDP_URL];
  }
  const hosts = LOOPBACK_ALIASES.includes(opts.host) ? ["127.0.0.1", "[::1]"] : [opts.host];
  return hosts.map((host) => `http://${host}:${opts.port}`);
}

const LOOPBACK_ALIASES = ["127.0.0.1", "localhost", "::1", "[::1]"];
