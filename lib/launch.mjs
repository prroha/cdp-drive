...
function holdsProfile(tokens, profile) {
  if (tokens.includes(`--user-data-dir=${profile}`)) {
    return true;
  }
  const separate = tokens.indexOf("--user-data-dir");
  return separate !== -1 && tokens[separate + 1] === profile;
}
...
export function profileHolders(processLines, profile) {
  return processLines
    ...
    .filter(Boolean)
    .map((line) => {
      const [pid, ...tokens] = line.split(/\s+/);
      return { pid: Number(pid), tokens };
    })
    .filter((entry) => Number.isFinite(entry.pid) && holdsProfile(entry.tokens, profile))
    .map((entry) => entry.pid);
}

// `ps` prints the argument list identically on macOS and Linux; `pgrep -a` does
// not -- on macOS it means "include ancestors" and prints bare pids, so every
// holder went undetected and the singleton files below were removed from a live
// profile.
function listProcesses() {
  try {
    return execFileSync("ps", ["-Ao", "pid=,args="], { encoding: "utf8" }).split("\n");
  } catch {
    return [];
  }
}
